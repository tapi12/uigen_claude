  ...
  if (messages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-4 text-center">
        ...
        <p className="text-neutral-900 font-semibold text-lg mb-2">¿Qué quieres crear hoy?</p>
        <p className="text-neutral-500 text-sm max-w-sm">Descríbeme lo que tienes en mente y lo construiré para ti</p>
      </div>
    );
  }
                          ...
                          case "reasoning":
                            return (
                              <div key={partIndex} className="mt-3 p-3 bg-white/50 rounded-md border border-neutral-200">
                                <span className="text-xs font-medium text-neutral-600 block mb-1">Analizando...</span>
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs font-mono border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">Listo</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-blue-600" />
                                    <span className="text-neutral-700">Trabajando...</span>
                                  </>
                                )}
                              </div>
                            );
